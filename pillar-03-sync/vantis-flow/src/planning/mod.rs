...
use crate::core::{Canvas, Color, Connection, ConnectionType, Element, ElementType, Style};
        ...
        for task in project.tasks.values() {
            for dep in &task.dependencies {
                if let (Some(source_task), Some(target_task)) =
                    (project.tasks.get(&dep.task_id), project.tasks.get(&task.id))
                {
                    if let (Some(_source_element), Some(_target_element)) = (
                        ...
                        let connection =
                            Connection::new(source_task.id, target_task.id, ConnectionType::Curved);

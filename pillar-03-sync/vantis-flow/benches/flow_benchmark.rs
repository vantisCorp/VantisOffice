...
use vantis_flow::{
    Canvas, Element, ElementType, Connection, ConnectionType, Style, Color, Stroke,
    MindMap,
    Flowchart, FlowchartNode, FlowchartNodeType,
    Task, TaskStatus, TaskPriority, Project, GanttChart, KanbanBoard,
};
use uuid::Uuid;
...
fn benchmark_canvas_creation(c: &mut Criterion) {
    ...
    group.bench_function("empty_canvas", |b| {
        b.iter(|| {
            black_box(Canvas::new("Test Canvas"))
        })
    });
...
fn benchmark_connection_creation(c: &mut Criterion) {
    ...
    for conn_type in connection_types {
        ...
        group.bench_with_input(BenchmarkId::new("type", name), &conn_type, |b, conn_type| {
            b.iter(|| {
...
fn benchmark_color_operations(c: &mut Criterion) {
    ...
    group.bench_function("create_color", |b| {
        b.iter(|| {
            black_box(Color::rgb(128, 64, 255))
        })
    });
    
    group.bench_function("from_hex", |b| {
        b.iter(|| {
            black_box(Color::from_hex("#FF5500").unwrap())
        })
    });
...
fn benchmark_flowchart_creation(c: &mut Criterion) {
    ...
    group.bench_function("flowchart_with_nodes", |b| {
        b.iter(|| {
            ...
            let process1 = FlowchartNode::new("Process 1", 100.0, 200.0, FlowchartNodeType::Process);
            flowchart.nodes.insert(process1.id, process1);
            
            let decision = FlowchartNode::new("Decision?", 100.0, 300.0, FlowchartNodeType::Decision);
            flowchart.nodes.insert(decision.id, decision);
            
            let process2 = FlowchartNode::new("Process 2", 100.0, 400.0, FlowchartNodeType::Process);
...
fn benchmark_flowchart_node_creation(c: &mut Criterion) {
    ...
    for node_type in node_types {
        ...
        group.bench_with_input(BenchmarkId::new("type", name), &node_type, |b, node_type| {
            b.iter(|| {
    ...
    group.bench_function("project_with_tasks", |b| {
        b.iter(|| {
            ...
            for i in 0..10 {
                ...
                task.set_priority(if i % 3 == 0 { TaskPriority::High } else { TaskPriority::Medium });
    ...
    group.bench_function("add_many_elements", |b| {
        b.iter(|| {
            ...
            for i in 0..100 {
                ...
                let element = Element::new(ElementType::Rectangle, x, y)
                    .with_text(format!("Element {}", i));
                canvas.add_element(element).unwrap();
            }
            black_box(canvas)
    ...
    group.bench_function("canvas_with_connections", |b| {
        b.iter(|| {
            ...
            for i in 0..element_ids.len() - 1 {
                let connection = Connection::new(element_ids[i], element_ids[i + 1], ConnectionType::Straight);

...
use crate::core::{Event, Calendar, DateRange, TimeRange, EventStatus};
...
impl Suggester {
    ...
    fn check_availability(&self, calendars: &[&Calendar], start: DateTime<Utc>, end: DateTime<Utc>) -> bool {
    ...
    fn calculate_score(&self, calendars: &[&Calendar], start: DateTime<Utc>, end: DateTime<Utc>, criteria: &SuggestionCriteria) -> f64 {
        let mut score = 0.0;
...
impl ConflictDetector {
    pub fn detect_conflicts(calendar: &Calendar) -> Vec<Conflict> {
        ...
        for i in 0..events.len() {
            for j in (i + 1)..events.len() {
                ...
                if event1.status == EventStatus::Confirmed && event2.status == EventStatus::Confirmed {
    ...
    fn calculate_severity(event1: &Event, event2: &Event) -> ConflictSeverity {
        ...
        let overlap_ratio = overlap_duration.num_minutes() as f64 / min_duration.num_minutes() as f64;

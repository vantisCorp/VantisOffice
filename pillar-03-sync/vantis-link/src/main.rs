...
use vantis_link::core::{ChangeType, UserRole};
use vantis_link::crdt::CrdtOperationType;
use vantis_link::{
    Change, CrdtEngine, CrdtType, Document, EncryptionAlgorithm, EncryptionManager, PeerDiscovery,
    Session, SyncManager, Transport, TransportProtocol, User,
};
...
fn main() {
    ...
    sync_manager
        .join_session(sync_session_id.clone(), user1.id.clone())
        .unwrap();
    println!("  ✓ User {} joined sync session", user1.name);

    sync_manager
        .join_session(sync_session_id.clone(), user2.id.clone())
        .unwrap();
    println!("  ✓ User {} joined sync session", user2.name);

    let sync_status = sync_manager
        .get_sync_status(sync_session_id.clone())
        .unwrap();
    println!("  ✓ Sync status: {:?}\n", sync_status);

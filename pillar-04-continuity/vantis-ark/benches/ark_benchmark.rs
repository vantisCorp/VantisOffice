...
use vantis_ark::core::{Backup, BackupManager, InMemoryStorage, Recovery, RecoveryStatus};
use vantis_ark::shamir::{PartVerifier, RecoverConfig, SecretSharing, SplitConfig};
...
fn benchmark_shamir_splitting(c: &mut Criterion) {
    ...
    for size in data_sizes {
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(format!("{}KB", size / 1024)),
            &size,
            |b, &_size| b.iter(|| black_box(SecretSharing::split(black_box(&data), &split_config))),
        );
    }
...
fn benchmark_shamir_recovery(c: &mut Criterion) {
    ...
    group.bench_function("recover_from_parts", |b| {
        b.iter(|| black_box(SecretSharing::recover(black_box(&parts), &recover_config).unwrap()))
    });
...
fn benchmark_serialization(c: &mut Criterion) {
    ...
    group.bench_function("serialize_backup", |b| {
        b.iter(|| black_box(serde_json::to_string(black_box(&backup)).unwrap()))
    });

    group.bench_function("deserialize_backup", |b| {
        let json = serde_json::to_string(&backup).unwrap();
        b.iter(|| black_box(serde_json::from_str::<Backup>(black_box(&json)).unwrap()))
    });
...
fn benchmark_backup_manager(c: &mut Criterion) {
    ...
    group.bench_function("create_backup", |b| {
        b.iter(|| {
            ...
            black_box(
                manager
                    .create_backup("Test Backup".to_string(), data)
                    .unwrap(),
            )
        })
    });

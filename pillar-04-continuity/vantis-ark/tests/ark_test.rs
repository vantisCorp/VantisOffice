...
use vantis_ark::{
    BackupConfig, BackupManager, InMemoryStorage, KeyManager, PartVerifier, RecoverConfig,
    RecoveryConfig, RetentionPolicy, SecretSharing, SplitConfig,
};
...
fn test_backup_creation() {
    ...
    let backup = manager
        .create_backup("test_backup".to_string(), data.clone())
        .unwrap();
...
fn test_full_backup_workflow() {
    ...
    let backup = manager
        .create_backup("workflow_test".to_string(), original_data.clone())
        .unwrap();

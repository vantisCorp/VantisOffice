...
use vantis_canvas::core::{Canvas, Shape, ShapeType, Text, Layer};
...
fn benchmark_canvas_creation(c: &mut Criterion) {
    ...
    group.bench_function("create_canvas", |b| {
        b.iter(|| {
            black_box(Canvas::new("Presentation".to_string()))
        })
    });
...
fn benchmark_shape_creation(c: &mut Criterion) {
    ...
    group.bench_function("create_rectangle", |b| {
        b.iter(|| {
            black_box(Shape::new(ShapeType::Rectangle))
        })
    });
    
    group.bench_function("create_circle", |b| {
        b.iter(|| {
            black_box(Shape::new(ShapeType::Circle))
        })
    });
    
    group.bench_function("create_triangle", |b| {
        b.iter(|| {
            black_box(Shape::new(ShapeType::Triangle))
        })
    });
    
    group.bench_function("create_star", |b| {
        b.iter(|| {
            black_box(Shape::new(ShapeType::Star { points: 5, inner_radius: 0.5 }))
        })
    });
...
fn benchmark_text_elements(c: &mut Criterion) {
    ...
    group.bench_function("create_text", |b| {
        b.iter(|| {
            black_box(Text::new("Sample text".to_string()))
        })
    });
...
fn benchmark_layer_operations(c: &mut Criterion) {
    ...
    group.bench_function("create_layer", |b| {
        b.iter(|| {
            black_box(Layer::new(0))
        })
    });
        ...
        b.iter(|| {
            ...
            for i in 0..100 {
                let shape_type = match i % 4 {
                    ...
                    _ => ShapeType::Star { points: 5, inner_radius: 0.5 },
                };
...
fn benchmark_serialization(c: &mut Criterion) {
    ...
    group.bench_function("serialize_simple_canvas", |b| {
        ...
        b.iter(|| {
            black_box(serde_json::to_string(black_box(&canvas)).unwrap())
        })
    });
    ...
    group.bench_function("deserialize_simple_canvas", |b| {
        ...
        b.iter(|| {
            black_box(serde_json::from_str::<Canvas>(black_box(&json)).unwrap())
        })
    });

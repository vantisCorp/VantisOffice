...
use vantis_writer::core::{Document, Paragraph, Editor, ParagraphStyle};
...
fn benchmark_live_preview(c: &mut Criterion) {
    ...
    group.bench_function("render", |b| {
        ...
        b.iter(|| {
            black_box(preview.render(markdown_text).unwrap())
        })
    });
...
fn benchmark_typography_render(c: &mut Criterion) {
    ...
    let medium_text = "The quick brown fox jumps over the lazy dog. Pack my box with five dozen liquor jugs.";
    let long_text = std::iter::repeat("The quick brown fox jumps over the lazy dog. ").take(10).collect::<String>();
    ...
    group.bench_function("short_text", |b| {
        b.iter(|| {
            black_box(engine.render_text(short_text, &(800.0, 600.0)).unwrap())
        })
    });
    
    group.bench_function("medium_text", |b| {
        b.iter(|| {
            black_box(engine.render_text(medium_text, &(800.0, 600.0)).unwrap())
        })
    });
    
    group.bench_function("long_text", |b| {
        b.iter(|| {
            black_box(engine.render_text(&long_text, &(800.0, 600.0)).unwrap())
        })
    });
...
fn benchmark_paragraph_style(c: &mut Criterion) {
    ...
    group.bench_function("default_style", |b| {
        b.iter(|| {
            black_box(ParagraphStyle::default())
        })
    });
...
fn benchmark_editor_operations(c: &mut Criterion) {
    ...
    group.bench_function("create_editor", |b| {
        ...
        b.iter(|| {
            black_box(Editor::new(doc.clone()))
        })
    });
    ...
    group.bench_function("insert_text", |b| {
        ...
        b.iter(|| {
            black_box(editor.insert_text("Hello, World!").unwrap())
        })
    });
    ...
    group.bench_function("delete_text", |b| {
        ...
        let mut editor = Editor::new(doc);
        b.iter(|| {
            black_box(editor.delete_text(10).unwrap())
        })
    });
...
fn benchmark_typography_settings(c: &mut Criterion) {
    ...
    group.bench_function("default_settings", |b| {
        b.iter(|| {
            black_box(TypographySettings::default())
        })
    });

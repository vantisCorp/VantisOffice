...
use vantis_grid::{
    CellValue, CollaborationManager, ExportFormat, FormulaEngine, Grid, NeuralEngine,
};
...
fn main() {
    ...
    grid.set_cell_value(0, 0, CellValue::Text("Month".to_string()))
        .unwrap();
    grid.set_cell_value(0, 1, CellValue::Text("Sales".to_string()))
        .unwrap();
    grid.set_cell_value(0, 2, CellValue::Text("Growth".to_string()))
        .unwrap();

    grid.set_cell_value(1, 0, CellValue::Text("January".to_string()))
        .unwrap();
    grid.set_cell_value(1, 1, CellValue::Number(10000.0))
        .unwrap();
    grid.set_cell_value(1, 2, CellValue::Number(0.05)).unwrap();

    grid.set_cell_value(2, 0, CellValue::Text("February".to_string()))
        .unwrap();
    grid.set_cell_value(2, 1, CellValue::Number(12000.0))
        .unwrap();
    grid.set_cell_value(2, 2, CellValue::Number(0.20)).unwrap();

    grid.set_cell_value(3, 0, CellValue::Text("March".to_string()))
        .unwrap();
    grid.set_cell_value(3, 1, CellValue::Number(15000.0))
        .unwrap();
    grid.set_cell_value(3, 2, CellValue::Number(0.25)).unwrap();

    grid.set_cell_value(4, 0, CellValue::Text("April".to_string()))
        .unwrap();
    grid.set_cell_value(4, 1, CellValue::Number(18000.0))
        .unwrap();
    grid.set_cell_value(4, 2, CellValue::Number(0.20)).unwrap();

    grid.set_cell_value(5, 0, CellValue::Text("May".to_string()))
        .unwrap();
    grid.set_cell_value(5, 1, CellValue::Number(22000.0))
        .unwrap();
    ...
    match engine.train_model("sales_model".to_string(), &sales_data) {
        Ok(_) => match engine.predict("sales_model", &[22000.0]) {
            Ok(prediction) => {
    ...
    impl vantis_grid::formulas::EvaluationContext for TestContext {
        fn get_cell_value(
            ...
            row: usize,
            column: usize,
        ) -> Result<CellValue, vantis_grid::formulas::FormulaError> {
    ...
    match collab_manager.create_session("demo_doc".to_string()) {
        Ok(session_id) => {
            ...
            match collab_manager.join_session(
                session_id.clone(),
                "user1".to_string(),
                "Alice".to_string(),
            ) {

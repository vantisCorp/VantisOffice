...
use crate::core::{parse_cell_reference, CellValue};
use chrono::Datelike;
use regex::Regex;
...
impl FormulaEngine {
    ...
    fn evaluate_function(
        ...
        // Evaluate arguments
        let evaluated_args: Result<Vec<CellValue>, FormulaError> = args
            .iter()
    ...
    fn evaluate_tokens(
        ...
        if values.len() == 1 {
            ...
            Err(FormulaError::EvaluationError(
...
impl Function for SqrtFunction {
    fn call(
        ...
        match &args[0] {
            ...
            CellValue::Number(_) => Err(FormulaError::EvaluationError(
...
impl Function for IfFunction {
    fn call(
        ...
        let condition = match &args[0] {
            ...
            _ => {
                return Err(FormulaError::TypeError(
                    "Expected boolean or number".to_string(),
                ))
            }
        };
    ...
    fn call(
        ...
        let result = args
            ...
            .map(|v| match v {
                ...
                CellValue::Boolean(b) => Ok::<String, FormulaError>(if *b {
                    "TRUE".to_string()
                } else {
                    "FALSE".to_string()
                }),

...
fn test_grid_basic_operations() {
    ...
    grid.set_cell_value(0, 2, CellValue::Text("Hello".to_string()))
        .unwrap();
...
fn test_collaboration_changes() {
    ...
    // Apply changes
    let change1 =
        vantis_grid::collaboration::CellChange::new("user1".to_string(), 0, 0, "100".to_string());
    let applied1 = manager.apply_change(session_id.clone(), change1).unwrap();
    assert_eq!(applied1.change.new_value, "100");

    let change2 =
        vantis_grid::collaboration::CellChange::new("user1".to_string(), 0, 1, "200".to_string());
...
fn test_export_csv() {
    ...
    grid.set_cell_value(0, 1, CellValue::Text("Name".to_string()))
        .unwrap();
    grid.set_cell_value(1, 0, CellValue::Number(2.0)).unwrap();
    grid.set_cell_value(1, 1, CellValue::Text("Value".to_string()))
        .unwrap();

...
use vantis_lens::{PdfDocument, PdfRenderer, PdfSterilizer, SignatureManager, AnnotationManager, ExportFormat};
...
fn main() {
    ...
    match sterilizer.sterilize(&mut document, &options) {
        Ok(report) => {
            ...
            println!("    External links removed: {}", report.external_links_removed);
    ...
    match sig_manager.create_signature(
        ...
        Ok(signature) => {
            ...
            println!("    Signer: {} <{}>", signature.signer_name, signature.signer_email);
    ...
    match ann_manager.create_annotation(AnnotationType::Highlight, 0) {
        Ok(mut annotation) => {
            annotation = annotation.with_content("Important text".to_string())
    ...
    match ann_manager.create_annotation(AnnotationType::Text, 1) {
        Ok(mut annotation) => {
            annotation = annotation.with_content("Note: This is important".to_string())

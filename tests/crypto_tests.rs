...
mod crypto_tests {
    ...
    fn test_encryption_different_keys() {
        let plaintext = TEST_MESSAGE;
        let key1 = generate_symmetric_key(32);
        let key2 = generate_symmetric_key(32);
        
        let ciphertext1 = simple_encrypt(plaintext, &key1);
        let ciphertext2 = simple_encrypt(plaintext, &key2);
    ...
    fn simple_decrypt(ciphertext: &[u8], key: &[u8]) -> Vec<u8> {

...
use flux_vector_engine::{
    capabilities,
    graphics::{Color, FillType, Paint, Path, VectorEngine},
    init,
    ui::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create UI widgets
    let mut button = Button::new("Click Me".to_string()).with_bounds(100.0, 300.0, 120.0, 40.0);

    let mut text_field =
        TextField::new("Enter text...".to_string()).with_bounds(100.0, 360.0, 200.0, 30.0);

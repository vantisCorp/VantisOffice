...
fn benchmark_decryption(c: &mut Criterion) {
    ...
    for size in sizes {
        let data = vec![b'x'; size];
        let encrypted = vault
            .encrypt_document(&data, EncryptionProfile::Software, KeySlot::Primary)
            .unwrap();
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &_size| {
            b.iter(|| {
                black_box(
                    vault
                        .decrypt_document(black_box(&encrypted), KeySlot::Primary)
                        .unwrap(),
                )
            })
        });
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &_size| {
            b.iter(|| {
                ...
                black_box(
                    vault
                        .decrypt_document(&encrypted, KeySlot::Primary)
                        .unwrap(),
                )
            })
        });
...
fn benchmark_key_slots(c: &mut Criterion) {
    ...
    group.bench_function("backup", |b| {
        b.iter(|| {
            let encrypted = vault
                .encrypt_document(&data, EncryptionProfile::Software, KeySlot::Backup)
                .unwrap();
    ...
    group.bench_function("custom", |b| {
        b.iter(|| {
            let encrypted = vault
                .encrypt_document(&data, EncryptionProfile::Software, KeySlot::Custom(1))
                .unwrap();
            black_box(
                vault
                    .decrypt_document(&encrypted, KeySlot::Custom(1))
                    .unwrap(),
            )
        })
    });
...
fn benchmark_vault_operations(c: &mut Criterion) {
    ...
    group.bench_function("create_vault", |b| {
        b.iter(|| black_box(Vault::new().unwrap()))
    });

    group.bench_function("default_vault", |b| b.iter(|| black_box(Vault::default())));
